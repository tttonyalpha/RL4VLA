...
def transform(processor, data):
    ...
    return dict(pixel_values=pixel_values, input_ids=input_ids, labels=labels, attention_mask=attention_mask,
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    with tqdm.tqdm(total=cfg.max_steps, leave=False) as progress:
        ...
        for epoch in range(cfg.epoch):
            for batch_idx, (batch_chosen, batch_rejected) in tqdm.tqdm(
                    enumerate(zip(dataloader_chosen, dataloader_rejected))):
                ...
                for traj_idx in range(traj_num):
                    ...
                    for step in range(8):
                        ...
                        with torch.autocast("cuda", dtype=torch.bfloat16):
                            ...
                            with torch.no_grad():
                                ...
                                multimodal_labels = torch.cat([labels[:, :1], projected_patch_labels, labels[:, 1:]],
                                                              dim=1)
                    ...
                    if distributed_state.is_main_process and gradient_step_idx % 1 == 0:
                        wandb.log(
                            {"train_loss": smoothened_loss, "chosen_rewards": chosen_rewards,
                             "reject_rewards": rejected_rewards, "reward_acc": reward_accuracies,
                             "reward_margins": reward_margins, "logps_rejected": logps_rejected,
                             "logps_chosen": logps_chosen}, step=gradient_step_idx
                        )
